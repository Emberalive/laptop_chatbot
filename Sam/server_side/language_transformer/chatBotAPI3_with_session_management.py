...
@app.get("/api/session")
async def get_or_create_session_endpoint(session_id: Optional[str] = None, user_id: Optional[str] = None):
...
@app.post("/api/session/replace")
async def replace_session(request: ResetRequest):
    """Replace the existing session with a new one."""
    try:
        ...
        new_session = get_or_create_session(user_id=user_id)
        return {"session_id": new_session.session_id, "replaced": True}
    except Exception as e:
        logger.error(f"Failed to replace session: {e}")
        raise HTTPException(status_code=500, detail=str(e))
